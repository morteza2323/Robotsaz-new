...
To copy an existing PostgreSQL database into a new one, set `SOURCE_DATABASE_URL` to the old database and `DESTINATION_DATABASE_URL` to the new database, then deploy or run `npm run db:transfer`. The application startup performs the same idempotent transfer from inside Liara's private network and preserves request order numbers. Once it succeeds, set `DATABASE_URL` to the destination URL, remove the two temporary transfer variables, and deploy again.
